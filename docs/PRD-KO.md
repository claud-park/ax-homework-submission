...
| 주요 행동 | Charter 작성 → Milestone 등록 → 산출물 업로드 → 댓글 응답 → 기한변경 요청 |
...
| 불편함 | 다수 챔피언의 비동기 제출물 트래킹 / 피드백 작성·전달 오버헤드 |
| 주요 행동 | 과제 생성 → Charter 리뷰 → 칸반 판정 → 댓글 작성 → 기한변경 승인 |
...
│  PostgreSQL (8 테이블 + partial index)│  │  6+ 트리거        │
│  Storage (submissions, deliverables)  │  └───────────────────┘
└───────────────────────────────────────┘
```
...
| 레이어 | 기술 | 역할 |
|---|---|---|
| 표현(Presentation) | React 18 + shadcn/ui + Tailwind CSS | 화면 구성·인터랙션 |
...
| 초안 전파 차단 | 초안 과제에 대한 Charter 제출·Milestone 등록·파일 업로드 차단 |
...
Charter 작성 (6 섹션) → 초안 저장 또는 게시
  ↓
WBS 마일스톤 등록 (주차·기간·제목) → Gantt 시각화
  ↓
과제 수행 중...
  ├─ 기한 부족 시 → 기한변경 요청 → (E2 어드민 알림)
  └─ 산출물 준비 시 → 파일 업로드 → milestone.status = completed → (E7 어드민 알림)
  ↓
...
```
[알림 수신] → /admin/kanban 접속
  ↓
검토중 카드 클릭 → 오른쪽 사이드 패널에서 제출물 확인
  ↓
댓글 작성 (선택) → (E4 챔피언 알림 자동 발송)
  ↓
카드 DnD → 합격 또는 불합격 컬럼으로 이동 (단방향·비가역)
```
...
| `milestones` | 챔피언의 WBS 항목 | id(PK), user_id, homework_id, week_number, start_date, due_date, status, publish_status |
...
| `milestone-deliverables` | `{user_id}/{milestone_id}/{filename}` | 비공개, 서명 URL (60s TTL) |
...
| DB / Auth | Supabase (관리형 PaaS) | 9개 마이그레이션 적용 완료 |
...
|---|---|---|
| 챔피언 | `/` | 과제 목록 |
| 챔피언 | `/homework/[id]` | 과제 상세 |
| 챔피언 | `/charter` | Charter 작성·제출 |
| 챔피언 | `/milestones` | WBS 관리 |
...
| 어드민 | `/admin/progress` | 챔피언 전체 진행 현황 (개발 중) |
| 어드민 | `/admin/reports` | 주간 리포트 (개발 중) |
...
| 그룹 | 엔드포인트 수 | 인증 방식 |
|---|---|---|
| 챔피언 API | 14 | `verifyJWT` |
| 어드민 API | 12 | `verifyJWT` + `verifyAdmin` |
| 인증 | 1 | OAuth 콜백 |
| **합계** | **27** | — |
...
### 11.1 완료 구간 (2026-05-14 ~ 2026-05-21)

| WBS # | 단계 | 작업 내용 | 공수 (MD) | 기간 | 상태 |
|---|---|---|:---:|---|---|
| 1.0 | **기반 구성** | DB 설계, Supabase 환경 세팅, Google OAuth 연동, 프로젝트 구조 초기화 | 2 | 05/14 | ✅ |
| 2.0 | **과제 관리 MVP** | 어드민 과제 CRUD (TipTap 에디터 포함), 챔피언 과제 목록 뷰, 파일 제출·Storage 연동, 칸반 DnD 판정 보드 | 5 | 05/14 | ✅ |
| 3.0 | **Charter 기능** | TipTap 6-섹션 WYSIWYG 편집기, 제출·재제출 API, 어드민 Charter 리뷰, Charter 댓글·답글 (양방향, max depth 2), DOCX 내보내기 | 4 | 05/15 | ✅ |
| 4.0 | **Milestone / WBS** | 챔피언 WBS CRUD, 자동 상태 계산 엔진, Gantt 시각화, 산출물 업로드, 기한변경 요청·승인 워크플로우 | 3 | 05/15 | ✅ |
| 5.0 | **이메일 알림 시스템** | Nodemailer + Gmail SMTP 연동, 7개 트리거 구현 (`notifyNewSubmission`, `notifyDeadlineChangeRequest`, `notifyNewComment` ×4, `notifyMilestoneCompleted`) | 2 | 05/18 | ✅ |
| 6.0 | **UI/UX 고도화** | shadcn/ui 컴포넌트 통일 (Dialog, AlertDialog, Sheet, Sonner), 리사이저블 사이드 패널, 낙관적 업데이트, 빈 상태 컴포넌트, 칸반 재설계 | 3 | 05/18 | ✅ |
| 7.0 | **임시저장 (Drafting)** | `publish_status` 스키마 마이그레이션 (3 테이블), 과제·Charter·Milestone 초안/게시 API, 초안 필터링 (칸반·진행도), 초안 전파 차단, 부분 인덱스 3개 | 3 | 05/19~21 | ✅ |
| 8.0 | **CI/CD 파이프라인** | GitHub Actions (Bun: lint·typecheck·build), Dockerfile (Next.js standalone), Docker Compose, Jenkins 배포 가이드, `.env` 보안 패턴 | 2 | 05/21 | ✅ |
| **완료 소계** | | | **24 MD** | | |

---

### 11.2 예정 구간 (2026-05-22 이후)

| WBS # | 단계 | 작업 내용 | 공수 (MD) | 예정 기간 | 우선순위 |
|---|---|---|:---:|---|---|
| 9.0 | **P0 — 안정화** | 판정 취소 API 도입, 이메일 `fire-and-forget` try-catch 래핑, CSS 변수 통일 (~7개 하드코딩 hex 제거), `unwrapSingle<T>` 헬퍼 추출, Gmail 앱 비밀번호 발급 절차 수립 | 3 | 1주 | 🔴 긴급 |
| 10.0 | **P1 — 진행 대시보드** | 챔피언 `/progress` 완성 (마일스톤 진행률·Gantt 요약), 어드민 `/admin/progress` 완성 (챔피언 전체 현황 테이블), 주간 리포트 `/admin/reports/[week]` | 6 | 2주 | 🟠 높음 |
| 11.0 | **P2 — 운영 확장** | 어드민 다중화 (과제별 담당자 매핑), 이메일 인프라 마이그레이션 (SendGrid 또는 SES), 발송 큐잉·재시도 로직 | 6 | 2주 | 🟡 보통 |
| 12.0 | **P3 — 분석·인사이트** | 챔피언 성과 분석 대시보드 (합격률·리드타임·재제출 패턴), 코칭 인사이트 리포트, In-app 알림 센터 (이메일 차단 대비) | 10 | 3주 | 🟢 낮음 |
| 13.0 | **P4 — 화이트라벨** | 프로그램 설정 관리 UI, 타 교육 프로그램 온보딩, 멀티-테넌트 기반 구조 | TBD | TBD | 📋 검토 |
| **예정 소계** | | | **25 MD** | | |
...
| # | 리스크 | 가능성 | 영향도 | 대응 방안 |
|---|---|---|---|---|
| R1 | Gmail SMTP 일일 발송 한도 (500건) 초과 | 중 | 중 | SendGrid / SES 마이그레이션 옵션 사전 준비 (P2) |
| R2 | 단일 어드민 메일박스 라우팅 한계 | 고 | 중 | 어드민 다중화 + 과제별 담당자 매핑 (P2) |
| R3 | 단방향 DnD — 오판정 복구 불가 | 중 | 고 | 어드민 전용 "판정 취소" API 도입 (P0) |
| R4 | 이메일 `fire-and-forget` 미처리 rejection | 중 | 저 | try-catch 래핑 + 에러 로깅 (P0) |
| R5 | Charter content JSONB 스키마 변경 시 마이그레이션 부담 | 저 | 중 | content 버전 필드 도입 + 점진적 마이그레이션 패턴 |
| R6 | Storage 용량 증가 (파일 누적) | 저 | 저 | 라이프사이클 정책·Cold Storage 전환 검토 |
| R7 | 챔피언 이메일 차단 시 알림 누락 | 중 | 중 | In-app 알림 센터 (P3) |
| R8 | Gmail 앱 비밀번호 노출 시 SMTP 탈취 | 저 | 고 | 서버 환경변수 격리, 정기 로테이션, SendGrid 전환 |
...
| 평균 댓글 응답 시간 | 부모 댓글 생성 → 답글 생성까지 경과 시간 | ≤ 12시간 |
...
| CI/CD Phase 1 계획 | `docs/superpowers/plans/2026-05-21-cicd-phase1-bun.md` | GitHub Actions 구성 |
| CI/CD Phase 2 계획 | `docs/superpowers/plans/2026-05-21-cicd-phase2-docker.md` | Docker 빌드 구성 |
| 임시저장 설계 스펙 | `docs/superpowers/specs/2026-05-19-drafting-feature-design.md` | Drafting 기능 설계 |
...
| 2026-05-20~21 | 임시저장 전체 구현 (API·UI·DB), CI/CD 파이프라인 완성, FK 버그 수정 |
