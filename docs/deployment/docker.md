...
| `CRON_SECRET` | ✗ | ✓ | daily-nudge 엔드포인트 인증. frontend·cron 사이드카가 공유 |
...
## 스케줄러 (daily-nudge cron 사이드카)

Vercel Cron을 떼어냈으므로(`vercel.json` 삭제) 매일 nudge 이메일은 **compose의 `cron` 사이드카**가 돌린다. `docker compose up -d` 하면 frontend와 함께 자동 기동되므로 별도 스케줄러 설정이 필요 없다.

- 사이드카(`alpine` + busybox `crond`)가 **매일 05:00 UTC(= 14:00 KST)** 에 frontend를 호출:
  `curl -H "Authorization: Bearer $CRON_SECRET" http://frontend:3000/api/cron/daily-nudge`
- `CRON_SECRET` 은 `.env` 의 frontend·cron 양쪽에 동일 값으로 주입된다. 라우트가 이 토큰으로 인증.
- crond는 컨테이너 로컬타임(UTC) 기준이라 `0 5 * * *` 가 곧 14:00 KST. 시각 변경은 compose의 `cron.command` crontab 라인에서.

```bash
# 사이드카 로그 / crontab 실행 기록
docker compose -p ax-homework-submission logs -f cron
docker compose -p ax-homework-submission exec cron cat /var/log/cron.log

# 수동 1회 실행 (스케줄 기다리지 않고 즉시 검증)
docker compose -p ax-homework-submission exec cron \
  curl -fsS -H "Authorization: Bearer $CRON_SECRET" http://frontend:3000/api/cron/daily-nudge
```
